...
export const LoginUser = async (req: Request, res: any) => {
    ...
    try {
        ...
        logger.error("Error during user login", { error: error instanceof Error ? error.message : "Unknown error" });
        return res.status(500).json({ success: false, message: "Internal Server Error" });
    }
}
...
export const handleRefreshToken = async (req: Request, res: any) => {
    ...
    try {
        ...
        logger.error("Error during token refresh", { error: error instanceof Error ? error.message : "Unknown error" });
        return res.status(500).json({ success: false, message: "Internal Server Error" });
    }
}

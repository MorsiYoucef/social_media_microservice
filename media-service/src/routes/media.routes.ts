    ...
    router.post("/upload", authenticateRequest, (req: Request, res: Response, next: NextFunction) =>{
        upload(req, res, (err) => {
            if (err instanceof multer.MulterError) {
                logger.error("Error during file upload", err);
                res.status(400).json({ success: false, message: "Invalid file format", error: err.message, stack: err.stack });
                return;
            }else if(err){
